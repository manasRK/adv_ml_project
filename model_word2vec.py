...
def read_labeled_doc(full_file_name):
    ...
    for entry in data:
        label = entry['uid'] + '_' + entry['pid']
        review = entry['rev']
        yield doc2vec.TaggedDocument(words=review.split(), tags=[label.encode('utf-8')])
...
if __name__ == '__main__':
    ...
    model.build_vocab(read_labeled_doc(path.join(util.data_path, util.file_name)))
    for i in xrange(0, 1):
